...
template <typename T>
struct has_ostream_operator<T, decltype(void(std::declval<std::ostream &>()
                                             << std::declval<const T &>()))>
   : std::true_type {
};
...
template <typename T> std::string toString(const T &v)
{
   if constexpr (has_ostream_operator<T>::value) {
      return static_cast<std::ostringstream &&>(
                (std::ostringstream() << std::boolalpha << v))
...
std::vector<std::string> inline split(const std::string &str)
{
   ...
   while (std::getline(ss, key, ',')) {
      if (!key.empty() && key.back() == '\0')
         key.pop_back(); // last variables contain a '\0', which is unexpected when
                         // comparing to raw string, e.g. value == "test" will fail when
                         // the last character is '\0'. Therefore we can remove it
...
template <> inline char get(const std::string &v)
{
   return v.empty()      ? throw std::invalid_argument("empty string")
          : v.size() > 1 ? v.substr(0, 2) == "0x" ? (char)std::stoul(v, nullptr, 16)
                                                  : (char)std::stoi(v)
                         : v[0];
}
...
template <> inline bool get(const std::string &v)
{
...
{ // remaining types
   if constexpr (is_vector<T>::value) {
      ...
      T res(splitted.size());
      if (!v.empty())
         std::transform(splitted.begin(), splitted.end(), res.begin(),
                        get<typename T::value_type>);
      return res;
   } else if constexpr (std::is_pointer<T>::value) {
      return new typename std::remove_pointer<T>::type(
         get<typename std::remove_pointer<T>::type>(v));
...
#else
      throw std::runtime_error("Enum not supported, please install magic_enum "
...
struct ConvertBase {
   ...
   virtual void set_default(const std::unique_ptr<ConvertBase> &default_value,
                            const std::string &default_string) = 0;
...
template <typename T> struct ConvertType : public ConvertBase {
   ...
   void convert(const std::string &v) override { data = get<T>(v); }

   void set_default(const std::unique_ptr<ConvertBase> &default_value,
                    const std::string &default_string) override
   {
      if (this->get_type_id() ==
          default_value->get_type_id()) // When the types do not match exactly. resort to
                                        // string conversion
   ...
   [[nodiscard]] size_t get_type_id() const override { return typeid(T).hash_code(); }
...
struct Entry {
   ...
   Entry(ARG_TYPE type, const std::string &key, std::string help,
         std::optional<std::string> implicit_value = std::nullopt)
      : type(type), keys_(split(key)), help(std::move(help)),
        implicit_value_(std::move(implicit_value))
   {
   }

   // Allow both string inputs and direct-type inputs. Where a string-input will be
   // converted like it would when using the commandline, and the direct approach is to
   // simply use the value provided.
   template <typename T> Entry &set_default(const T &default_value)
   {
      this->default_str_ = toString(default_value);
      if constexpr (!(std::is_array<T>::value ||
                      std::is_same<typename std::remove_all_extents<T>::type,
                                   char>::value)) {
   ...
   {
      // Automatically set the default to nullptr for pointer types and empty for optional
      // types
      if constexpr (is_optional<T>::value || std::is_pointer<T>::value ||
   ...
   template <typename T>
   operator T() {} // When you get here  because you received an error, make sure all
                   // parameters of argparse are references (e.g. with `&`)
 ...
 private:
   ...
   [[nodiscard]] std::string _get_keys() const
   {
      ...
      for (size_t i = 0; i < keys_.size(); i++)
         ss << (i ? "," : "")
            << (type == ARG ? "" : (keys_[i].size() > 1 ? "--" : "-")) + keys_[i];
      return ss.str();
   }
   ...
   void _convert(const std::string &value)
   {
      try {
         ...
         error = "Invalid argument, could not convert \"" + value + "\" for " +
                 _get_keys() + " (" + help + ")";
      }
      catch (const std::runtime_error &e) {
         error = "Invalid argument \"" + value + "\" for " + _get_keys() + " (" + help +
                 "). Error: " + e.what();
      }
   }
   ...
   void _apply_default()
   {
      ...
      } else if (default_str_.has_value()) { // in cases where a string is provided to the
   ...
   [[nodiscard]] std::string info() const
   {
      ...
      const std::string default_value =
         default_str_.has_value() ? "default: " + *default_str_ : "required";
      const std::string implicit_value =
         implicit_value_.has_value() ? "implicit: \"" + *implicit_value_ + "\", " : "";
      const std::string allowed_value =
         !allowed_entries.empty()
            ? "allowed: <" + allowed_entries.substr(0, allowed_entries.size() - 2) + ">, "
            : "";
...
class Args
{
 ...
 public:
   ...
   /* Add a positional argument, the order in which it is defined equals the order in
    * which they are being read. help : Description of the variable
    *
    * Returns a reference to the Entry, which will collapse into the requested type in
    * `Entry::operator T()`
    */
   Entry &arg(const std::string &help)
   {
      std::shared_ptr<Entry> entry =
         std::make_shared<Entry>(Entry::ARG, "arg_" + std::to_string(_arg_idx++), help);
    ...
    * key : A comma-separated string, e.g. "k,key", which denotes the short (-k) and
    * long(--key) keys_ help : Description of the variable implicit_value : Implicit
    * values are used when no value is provided.
    *
    * Returns a reference to the Entry, which will collapse into the requested type in
    * `Entry::operator T()`
    */
   Entry &kwarg(const std::string &key, const std::string &help,
                const std::optional<std::string> &implicit_value = std::nullopt)
   {
      std::shared_ptr<Entry> entry =
         std::make_shared<Entry>(Entry::KWARG, key, help, implicit_value);
      all_entries.emplace_back(entry);
    ...
    * key : A comma-separated string, e.g. "k,key", which denotes the short (-k) and
    * long(--key) keys_ help : Description of the variable
    *
   ...
   virtual void welcome() {} // Allow to overwrite the `welcome` function to add a
                             // welcome-message to the help output
   virtual void help()
   {
      welcome();
      ...
      cout << " [options...]" << endl;
      for (const auto &entry : arg_entries) {
         cout << setw(17) << entry->keys_[0] << " : " << entry->help << entry->info()
              << endl;
      }
      ...
      for (const auto &entry : all_entries) {
         if (entry->type != Entry::ARG) {
            cout << setw(17) << entry->_get_keys() << " : " << entry->help
                 << entry->info() << endl;
         }
      }
   }
   ...
   /* parse all parameters and also check for the help_flag which was set in this
    * constructor Upon error, it will print the error and exit immediately.
    */
   void parse(int argc, const char *const *argv, const bool &raise_on_error)
   {
      ...
      auto is_value = [&](const size_t &i) -> bool {
         return params.size() > i &&
                (params[i][0] != '-' ||
                 (params[i].size() > 1 &&
                  std::isdigit(params[i][1]))); // check for number to not accidentally
                                                // mark negative numbers as non-parameter
      };
      auto parse_param = [&](size_t &i, const std::string &key, const bool is_short,
                             const std::optional<std::string> &equal_value =
                                std::nullopt) {
         ...
         if (itt != kwarg_entries.end()) {
            ...
            } else if (!is_short) { // short values are not allowed to look ahead for the
                                    // next parameter
               if (is_value(i + 1)) {
                  ...
                  entry->_convert(""); // for multiargument parameters, return an empty
                                       // vector when not passing any more values
               } else {
      ...
      auto add_param = [&](size_t &i, const size_t &start) {
         size_t eq_idx =
            params[i].find('='); // check if value was passed using the '=' sign
      ...
      // Parse all the positional arguments, making sure multi_argument positional
      // arguments are processed last to enable arguments afterwards
      size_t arg_i = 0;
      for (; arg_i < arg_entries.size() && !arg_entries[arg_i]->_is_multi_argument;
           arg_i++) { // iterate over positional arguments until a multi-argument is found
         if (arg_i < arguments_flat.size())
            arg_entries[arg_i]->_convert(arguments_flat[arg_i]);
      }
      size_t arg_j = 1;
      for (size_t j_end = arg_entries.size() - arg_i; arg_j <= j_end;
           arg_j++) { // iterate from back to front, to ensure non-multi-arguments in the
                      // front and back are given preference
         ...
         if (flat_idx < arguments_flat.size() && flat_idx >= arg_i) {
            if (arg_entries[arg_entries.size() - arg_j]->_is_multi_argument) {
               std::stringstream s; // Combine multiple arguments into 1 comma-separated
                                    // string for parsing
               copy(&arguments_flat[arg_i], &arguments_flat[flat_idx + 1],
                    std::ostream_iterator<std::string>(s, ","));
               std::string value = s.str();
               value.back() = '\0'; // remove trailing ','
               arg_entries[arg_i]->_convert(value);
            } else {
               arg_entries[arg_entries.size() - arg_j]->_convert(
                  arguments_flat[flat_idx]);
            }
         }
      }
   ...
   void print() const
   {
      for (const auto &entry : all_entries) {
         std::string snip =
            entry->type == Entry::ARG
               ? "(" +
                    (entry->help.size() > 10 ? entry->help.substr(0, 7) + "..."
                                             : entry->help) +
                    ")"
               : "";
         cout << setw(21) << entry->_get_keys() + snip << " : "
              << (entry->is_set_by_user ? bold(entry->value_.value_or("null"))
                                        : entry->value_.value_or("null"))
              << endl;
      }
   }
};
