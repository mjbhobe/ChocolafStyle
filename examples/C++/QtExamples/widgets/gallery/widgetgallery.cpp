...
WidgetGallery::WidgetGallery(QWidget *parent) : QDialog(parent), progressBar(createProgressBar())
{
  ...
  auto helpLabel = createWidget1<QLabel>(tr("Press F1 over a widget to see Documentation"),
                                         "helpLabel");

  auto disableWidgetsCheckBox = createWidget1<QCheckBox>(tr("&Disable widgets"),
                                                         "disableWidgetsCheckBox");
  ...
  connect(styleComboBox, &QComboBox::textActivated, this, &WidgetGallery::changeStyle);
  ...
  connect(disableWidgetsCheckBox,
          &QCheckBox::toggled,
          simpleInputWidgetsGroupBox,
          &QWidget::setDisabled);
  ...
  auto dialogButtonBox = createWidget1<QDialogButtonBox>(QDialogButtonBox::Help
...
QGroupBox *WidgetGallery::createButtonsGroupBox()
{
  ...
  auto defaultPushButton = createWidget1<QPushButton>(tr("Default Push Button"),
                                                      "defaultPushButton");
  ...
  auto commandLinkButton = createWidget1<QCommandLinkButton>(tr("Command Link Button"),
                                                             "commandLinkButton");
...
QTabWidget *WidgetGallery::createItemViewTabWidget()
{
  ...
  listModel->appendRow(
      new QStandardItem(QIcon(QLatin1String(
...
void WidgetGallery::updateSystemInfo()
{
  ...
  for (auto screen : screens) {
    ...
    str << '"' << screen->name() << "\" " << screen->geometry() << ", "
        << screen->logicalDotsPerInchX() << "DPI, DPR=" << screen->devicePixelRatio();
    if (current)
      str << "</i>";
    str << "</li>";
  }
