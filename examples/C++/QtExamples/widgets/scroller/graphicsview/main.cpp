...
/*!
    \class RectObject
    Note that it needs to be a QGraphicsObject or else the gestures will not
   work correctly.
*/
...
public:
   RectObject(const QString &text, qreal x, qreal y, qreal width, qreal height,
              QBrush brush, QGraphicsItem *parent = nullptr)
      : QGraphicsObject(parent), m_text(text), m_rect(x, y, width, height),
        m_pen(brush.color().lighter(), 3.0), m_brush(brush)
   {
   ...
   {
      // here we only want the size of the children and not the size of the
...
class ViewObject : public QGraphicsObject {
   Q_OBJECT
public:
   ViewObject(QGraphicsObject *parent) : QGraphicsObject(parent) {}
...
public:
   ListObject(const QSizeF &size, bool useTouch)
   {
      ...
      QScroller::grabGesture(this,
                             useTouch ? QScroller::TouchGesture
   ...
   bool event(QEvent *e) override
   {
      ...
      case QEvent::ScrollPrepare: {
         ...
         se->setContentPosRange(QRectF(0, 0, qMax(qreal(0), br.width() - m_size.width()),
...
public:
   MainWindow(bool useTouch)
   {
      ...
      for (int i = 0; i < NUM_LISTS; i++) {
         ListObject *childList = new ListObject(QSizeF(mainList->m_size.width() / 3,
                                                       mainList->m_size.height()),
                                                useTouch);
    ...
    */
      ...
      m_scene->setSceneRect(0, 0, m_view->viewport()->width(),
   ...
   */
   void fillList(ListObject *list)
   {
      ...
      for (int i = 0; i < NUM_ITEMS; i++) {
         QColor color = QColor(255 * i / NUM_ITEMS, 255 * (NUM_ITEMS - i) / NUM_ITEMS,
                               127 * (i % 2) + 64 * (i / 2 % 2));
         QString text = QLatin1String("Item #") + QString::number(i);
         QGraphicsItem *rect = new RectObject(text, 0, 0, list->m_size.width() - 6, h - 3,
                                              QBrush(color), list->viewport());
         rect->setPos(3, h * i + 3);
      }
