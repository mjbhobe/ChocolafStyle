...
class RectObject : public QGraphicsObject {
    Q_OBJECT
  public:
    RectObject(const QString &text, qreal x, qreal y, qreal width, qreal height, QBrush brush,
...
class ListObject : public QGraphicsObject {
  ...
  public:
    ...
    QRectF boundingRect() const override { return QRectF(0, 0, m_size.width() + 3, m_size.height()); }
  ...
  public:
    MainWindow(bool useTouch)
    {
      ...
      for (int i = 0; i < NUM_LISTS; i++) {
        ListObject *childList =
            new ListObject(QSizeF(mainList->m_size.width() / 3, mainList->m_size.height()), useTouch);
    ...
    */
    ...
    void fillList(ListObject *list)
    {
      ...
      for (int i = 0; i < NUM_ITEMS; i++) {
        QColor color =
            QColor(255 * i / NUM_ITEMS, 255 * (NUM_ITEMS - i) / NUM_ITEMS, 127 * (i % 2) + 64 * (i / 2 % 2));
        QString text = QLatin1String("Item #") + QString::number(i);
        QGraphicsItem *rect =
            new RectObject(text, 0, 0, list->m_size.width() - 6, h - 3, QBrush(color), list->viewport());
        rect->setPos(3, h * i + 3);
      }
