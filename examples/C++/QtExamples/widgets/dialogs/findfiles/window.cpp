...
void Window::browse()
{
  QString directory = QDir::toNativeSeparators(
      QFileDialog::getExistingDirectory(this, tr("Find Files"), QDir::currentPath()));
...
QStringList Window::findFiles(const QStringList &files, const QString &text)
{
  ...
  for (int i = 0; i < files.size(); ++i) {
    ...
    progressDialog.setLabelText(
        tr("Searching file number %1 of %n...", nullptr, files.size()).arg(i));
...
void Window::showFiles(const QStringList &paths)
{
  ...
  filesFoundLabel->setText(
      tr("%n file(s) found (Double click on a file to open it)", nullptr, paths.size()));
