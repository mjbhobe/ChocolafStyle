...
ImageSpinner::ImageSpinner(const QString &imagePath)
  : m_currIndex(-1)
{
  QFileInfo current(imagePath);
  m_dir = current.absoluteDir();
  QStringList imageFilters;
  imageFilters << "*.tiff" << "*.tif" << "*.jpg" << "*.jpeg" << "*.gif"
      << "*.bmp" << "*.png";
...
QString ImageSpinner::prevImage()
{
  ...
  QString prevImagePath = m_dir.absolutePath() + QDir::separator() + m_fileNames.at(
    m_currIndex);
...
QString ImageSpinner::nextImage()
{
  ...
  QString nextImagePath = m_dir.absolutePath() + QDir::separator() + m_fileNames.at(
    m_currIndex);
