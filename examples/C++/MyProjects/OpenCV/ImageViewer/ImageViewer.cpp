...
void ImageViewer::createActions()
{
    ...
    rotateLeftAction->setStatusTip(
        "Rotate image counter-clockwise by 90 degrees");
    QObject::connect(rotateLeftAction, SIGNAL(triggered()), this,
        SLOT(rotateLeft()));
    ...
    QObject::connect(rotateRightAction, SIGNAL(triggered()), this,
        SLOT(rotateRight()));
    ...
    QObject::connect(zoomNormalAction, SIGNAL(triggered()), this,
        SLOT(normalSize()));
    ...
    QObject::connect(fitToWindowAction, SIGNAL(triggered()), this,
        SLOT(fitToWindow()));
    fitToWindowAction->setEnabled(false);
    fitToWindowAction->setCheckable(true);

    prevImageAction = new QAction("&Previous image", this);
    prevImageAction->setShortcut(
        QKeySequence::MoveToPreviousChar); // left arrow (usually)
    ...
    QObject::connect(prevImageAction, SIGNAL(triggered()), this,
        SLOT(prevImage()));
    prevImageAction->setEnabled(false);

    nextImageAction = new QAction("&Next image", this);
    nextImageAction->setShortcut(
        QKeySequence::MoveToNextChar); // right arrow (usually)
    ...
    QObject::connect(nextImageAction, SIGNAL(triggered()), this,
        SLOT(nextImage()));
    ...
    QFileDialog::AcceptMode acceptMode)
{
    ...
    dialog.setDirectory(picsLocation.isEmpty() ? QDir::currentPath()
                                               : picsLocation.last());
...
void ImageViewer::print()
{
    QMessageBox::information(this, "ImageViewer",
        "TODO: Implementation for Print Image");
}
...
void ImageViewer::prevImage()
{
    if (!imageSpinner->atFirst()) {
        ...
        if (imageSpinner->atFirst())
            QMessageBox::information(this, "ImageViewer",
                "Displaying first image in folder!");
    }
}
...
void ImageViewer::nextImage()
{
    if (!imageSpinner->atLast()) {
        ...
        if (imageSpinner->atLast())
            QMessageBox::information(this, "ImageViewer",
                "Displaying last image in folder!");
    }
}
