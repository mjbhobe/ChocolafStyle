  ...
  public:
     ...
     */
      ...
      for (size_t i = num_periods + 1; i < prices.size(); ++i) {
        T val = static_cast<T>((1 - multiplier) * ema.back() + multiplier * prices[i]);
        ema.push_back(val);
      }
      return ema;
    }
    ...
    {
      return (std::accumulate(prices.cbegin(), prices.cend(), 0.0)) / prices.size();
    }
    ...
    static double corr(const std::vector<T> &prices1, const std::vector<T> &prices2)
    {
