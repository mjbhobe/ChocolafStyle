...
#ifdef Q_OS_WINDOWS
const QString CODE_DIR =
    QString("C:/Dev/Code/git-projects/ChocolafStyle/examples/C++/MyProjects");
#else
const QString CODE_DIR = QString(
    "/home/mjbhobe/code/git-projects/ChocolafStyle/examples/C++/MyProjects");
#endif
...
int main(int argc, char *argv[]) {
  ...
  if (!dir.exists()) {
    cerr << "FATAL: search directory " << dir.path() << " does not exist!"
         << Qt::endl;
    return EXIT_FAILURE;
  } else {
    ...
    // display the files
    cout << "Found " << matchingFiles.length()
         << " files with extensions: " << suffix << Qt::endl;
