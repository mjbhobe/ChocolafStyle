...
QSqlDatabase connectToDatabase() {
  ...
  cout << "Connected to PostgreSQL db " << db.databaseName().toStdString()
       << endl
       << flush;
  return db;
}
...
int main(int argc, char **argv) {
  ...
  if (!conn.is_open()) {
    cerr << "FATAL ERROR: unable to connect to database " << conn.dbname()
         << endl;
    return -1;
  }
  ...
  try {
    ...
    std::string winTitle =
        fmt::format("Welcome to the QtFishingStore using Qt {} C++ for Windows",
                    QT_VERSION_STR);
