...
void FishingStoreSetupDb::populateDb(pqxx::connection &conn)
{
   ...
   try {
      ...
      std::vector<customer_data_rec>
         customer_recs{{"James", "Smith", "", "NULL"},
                       {"Mary", "Johnson", "", "NULL"},
                       {"John", "Williams", "", "NULL"},
      ...
      for (const auto &[first_name, last_name, email, phone] : customer_recs) {
         txn.exec_prepared("insert_customers", first_name, last_name, email, phone);
         num_recs++;
      }
      ...
      std::vector<stores_data_rec> store_recs{{"Boston Fish Supplies", "(617) 987-6543",
                                               "MA"},
                                              {"Miami Fish Supplies", "(786) 123-4567",
                                               "FL"}};
