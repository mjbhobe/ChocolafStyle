...
auto mean(auto a, auto b, auto c) { return (a + b + c) / 3; }
...
void templ_ex1()
{
  ...
  /*std::cout << */ std::println(
      "d: {:.3f} - e: {:.3f} - f: {:.3f} - d + e + f: {:.3f}", d, e, f, add_values(d, e, f));

  // adding strings
  std::string h{"Hello "}, i{"C++ "}, j{"23. You rock!"};
  std::cout << std::format(
      "h: \"{}\" - i: \"{}\" - j: \"{}\" - h + i + j: \"{}\"\n", h, i, j, add_values(h, i, j));
...
void templ_ex2()
{
  ...
  // add & display
  std::println("p1: {} - p2: {} - p3: {} - p1+p2+p3: {}", p1, p2, p3, add_values(p1, p2, p3));
