...
namespace loguru
{
    ...
    /* Everything with a verbosity equal or greater than g_stderr_verbosity will be
     w ritten to stderr. You can set this in code* or via the -v argument.
    ...
    /* Specify the verbosity used by loguru to log its info messages including the header
     l ogged when logged::init() is called or on *exit. Default is 0 (INFO).
     */
    ...
    /*  Should be called from the main thread.
     Y ou don't *need* to call this, but if you *do you get:
    ...
    /* Returns the name of the app as given in argv[0] but without leading path.
     T hat is, if argv[0] is "../foo/app" this* will return "app".
     */
    LOGURU_EXPORT
    ...
    /* Given a prefix of e.g. "~/loguru/" this might return
     " /home/your_username/loguru/app_name/201*51017_161503.123.log"
    ...
    /*  Will log to a file at the given path.
     A ny logging message with a verbosity lower* or equal to
    ...
    /*  Will be called right before abort().
     Y ou can for instance use this to print cus*tom error messages, or throw an exception.
    ...
    /*  Will be called on each log messages with a verbosity less or equal to the given one.
     U seful for displaying messages on-screen i*n a game, for example.
    ...
    /*  Set a callback that returns custom verbosity level names. If callback
     i s nullptr or returns nullptr, default log* names will be used.
     */
    LOGURU_EXPORT
    ...
    /*  Set a callback that returns the verbosity level matching a name. The
     c allback should return Verbosity_INVALID i*f the name is not
     recognized.
     */
    LOGURU_EXPORT
    ...
    /*  Get the verbosity enum value from a custom 4-character level name, if one exists.
     I f the name does not match a custom level *name, Verbosity_INVALID is returned.
     */
    LOGURU_EXPORT
    ...
    /* Thread names can be set for the benefit of readable logs.
     I f you do not set the thread name, a hex* id will be shown instead.
    ...
    /* Returns the thread name for this thread.
     O n most *nix systems this will return th*e system thread name (settable from both within and without Loguru).
    ...
    /* Generates a readable stacktrace as a string.
     ' skip' specifies how many stack frames t*o skip.
     ...
     *
     F or instance, instead of having a stack tr*ace look like this:
    ...
    // ------------------------------------------------------------------------
    /*
     N ot all terminals support colors, but if th*ey do, and g_colorlogtostderr
    ...
    /* 	A stack trace gives you the names of the function at the point of a crash.
     W ith ERROR_CONTEXT, you can also get the v*alues of select local variables.
     Usage:
...
*/
    ...
    /*
     G et a light-weight handle to the error con*text stack on this thread.
...
*/
    ...
    /*
     Y ou can add ERROR_CONTEXT support for your *own types by overloading ec_to_text. Here's how:
...
/* Checked at runtime too. Will print error, then call fatal_handler (if any), then 'abort'.
 N ote that the test must be boolean.      *
...
/* This file extends loguru to enable std::stream-style logging, a la Glog.
 I t's an optional feature behind the LOGUR*U_WITH_STREAMS settings
...
namespace loguru
{
    ...
    /*  Helper functions for CHECK_OP_S macro.
     G LOG trick: The (int, int) specialization *works around the issue that the compiler
