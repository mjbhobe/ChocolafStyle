...
int main() {
   cout << "[with cout] - Hello World, welcome to the Qt Framework!" << Qt::endl;
   // we can easily mix std C++ with Qt
   cout <<
      std::format("[with std::format()] - You are using Qt Framework version {}", QT_VERSION_STR).c_str()
      << Qt::endl;
   //cout << "You are using Qt Framework version " << QT_VERSION_STR << Qt::endl;
   // same line using C++23 std::println()
   std::println("[with std::println()] - You are using Qt Framework version {}", QT_VERSION_STR);
