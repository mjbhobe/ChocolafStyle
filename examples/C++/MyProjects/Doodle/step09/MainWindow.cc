...
const QString WindowTitle =
    QString("Qt %1 Doodle - Step09: Building a concious GUI").arg(QT_VERSION_STR);
...
MainWindow::MainWindow()
{
  ...
  QObject::connect(_drawWindow, SIGNAL(doodleModified2(bool)), this,
...
void MainWindow::createActions()
{
  ...
  QObject::connect(fileSaveAsAction, SIGNAL(triggered()), _drawWindow,
                   SLOT(fileSaveAs()));
  ...
  penWidthAction =
      new QAction(QIcon(":/icons/penWidth.png"), tr("Change pen &width..."), this);
  penWidthAction->setShortcut(tr("Ctrl+W"));
  penWidthAction->setStatusTip(tr("Change the width of default pen."));
  QObject::connect(penWidthAction, SIGNAL(triggered()), _drawWindow,
                   SLOT(changePenWidth()));

  penColorAction =
      new QAction(QIcon(":/icons/penColor.png"), tr("Change pen &color..."), this);
  penColorAction->setShortcut(tr("Ctrl+L"));
  penColorAction->setStatusTip(tr("Change the color of default pen."));
  QObject::connect(penColorAction, SIGNAL(triggered()), _drawWindow,
                   SLOT(changePenColor()));
...
void MainWindow::about()
{
  ...
  ostr << "<html><b>Qt Scribble</b> - Doodling application<p/>Developed with the Qt "
       << QT_VERSION_STR << " C++ framework.<p/><p/>Written by - Manish Bhobe.<p/><p/>"
