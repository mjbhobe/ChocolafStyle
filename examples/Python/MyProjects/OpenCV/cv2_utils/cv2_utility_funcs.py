...
def translate_image(image: np.ndarray, cx: int, cy: int) -> np.ndarray:
    ...
    image_translated = cv2.warpAffine(
        image, translation_matrix, (num_cols + cx, num_rows + cy)
    )
...
def rotate_image(
    ...
    rotation_matrix = cv2.getRotationMatrix2D(
        (cx_rotate, cy_rotate), angle_degrees, scale_factor
    )
...
def scale_image(
    ...
    image_scaled = cv2.resize(
        image, None, fx=scalex, fy=scaley, interpolation=interpolation
    )
    return image_scaled
...
def perspective_transform_image(
    ...
    image_transformed = cv2.warpPerspective(
        image, perspective_matrix, (num_cols, num_rows)
    )
    return image_transformed
