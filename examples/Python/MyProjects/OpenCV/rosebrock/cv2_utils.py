...
"""
* cv2_utils.py - OpenCV utilities for images
*
* @author: Manish Bhobe
...
def translate_image(image, cx, cy):
    ...
    translation_matrix = np.float32([[1, 0, cx], [0, 1, cy]])
    image_translated = cv2.warpAffine(image, translation_matrix, (num_cols + cx, num_rows + cy))
    return image_translated


def rotate_image(image, angle_degrees, scale_factor = 1, cx = None, cy = None):
...
def scale_image(image, scalex, scaley, interpolation = cv2.INTER_LINEAR):
...
def affine_transform_image(image, src_points: np.ndarray, dst_points: np.ndarray):
...
def perspective_transform_image(image, src_points: np.ndarray, dst_points: np.ndarray):
